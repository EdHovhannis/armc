...
**Экспортируемые remote-модули (Module Federation `exposes`):** `./app` → `src/app.ts` (bridge-компонент приложения) и `./header` → `src/header.ts` (дефолт-экспорт `Widgets/Header`). Хедер вынесен в отдельный модуль - `app.ts` его больше не экспортирует, из `mfe-config.*` убран флаг `hasNewHeader`. `exposes` заданы в обоих rspack-конфигах.
...
- ~~Снять fallback на моки + `mock.ts`, повесить `failData → handleErrorFx`~~ - **сделано**: `Entities/Restriction` переведён на боевой паттерн, `mock.ts` удалён, dev работает через API-моки (`ARMC_MOCK`).
...
**Метки (`LabelsModal`, `Entities/Label`):** `TODO(labels)` - уточнить регистрозависимость у аналитика. (Сетевой fallback снят - `saveLabelsFx` на боевом `handleErrorFx`.)

**Остатки заглушек** (это **не** error-fallback, а заглушки тела запроса - снять при появлении контракта): MOCK-тела в `Entities/Limits` и `Steps/api.ts` (`createSchemaFx`).

> **Все «тихие» error-fallback'и сняты.** `Entities/Instance`, `Entities/Overdraft`, `Entities/Label`, `Entities/Restriction`, `deleteArchiveFx` переведены на боевой паттерн (`failData → handleErrorFx`); бывшие `TODO(instance-api)`/`TODO(overdraft-api)`/`TODO(labels-api)`/`TODO(archive-delete-api)`/`TODO(restrictions-api)` (в части fallback) закрыты. Исключения сознательные: массовый старт/стоп показывает провалы агрегированным тостом (не через handleErrorFx), чтение статуса экземпляра - best-effort. **Dev держится на API-моках (`ARMC_MOCK=true`)**, а не на проглатывании ошибок - при снятии fallback'ов добавлены недостающие mock-роуты (метки PUT, значение/PATCH/DELETE ограничения, добавление экземпляра).

**Не реализовано:** иконка-индикатор овердрафта в строке (зелёная/оранжевая/красная); сортировка по клику на заголовок столбца (контракт `paginated` на sort неизвестен - abyss не слал, надо снять со стенда; подробности в разделе «Drawer фильтров»). Заголовочный фильтр-воронка - **уже сделан** (второй вход в серверный фильтр уровня 1, см. там же); локальный фильтр уровня 2 по загруженным строкам сознательно не делаем.
...
Приложение — remote Module Federation, **standalone не рендерится** (`app.ts` экспортирует только bridge-компонент приложения; хедер - отдельный модуль `src/header.ts`). Для визуальной проверки нужен **host**.
...
### Сетевой слой: боевой паттерн + API-моки для офлайна

Раньше в проекте был «черновой» паттерн (эффект шлёт реальный запрос, при ошибке тихо возвращает мок, `handleErrorFx` не подключён). **От него отказались** - все эффекты теперь на стандартном «Типичном паттерне API» выше (`failData → handleErrorFx`), а офлайн-вёрстка без стенда держится на **API-моках** (`ARMC_MOCK=true`, см. «Локальные API-моки»): mock-слой перехватывает запрос и отдаёт фикстуру, поэтому `handleErrorFx` не срабатывает, а реальные ошибки на стенде - всплывают.

Поэтому новый эндпоинт готовим так: пишем эффект по «Типичному паттерну API» + при необходимости добавляем mock-роут в `Shared/api/mock`. Тихий `try/catch`-fallback внутри эффекта больше не используем (исключения - осознанная best-effort деградация, напр. чтение статуса экземпляра).
...
- типы payload или обработчики разные - цепочка `.on(...).on(...)`:
  ```ts
  $store.on(onChangeName, (_, value) => value).on(fetchSomethingFx.doneData, (_, response) => response.data);
  ```
...
- **Грабля (жертва мержа):** `__ARMC_MOCK__` обязан быть в `DefinePlugin` **обоих** rspack-конфигов (dev и prod) синхронно с `if (__ARMC_MOCK__)` в `axios.ts`. `tsc` пропажу не ловит - `global.d.ts` объявляет глобал только как тип, в рантайме его нет. Если `DefinePlugin` перестанет подставлять флаг (напр. потеряли строку при разрешении конфликта), `axios.ts` кинет `ReferenceError: __ARMC_MOCK__ is not defined` на старте, и весь MFE не загрузится. Правишь одно - проверь второе.
...
- **Маршруты** (`mock/index.ts`, массив `routes`): список конфигураций (`archive/list/paginated`, `archive/list/page-count`), `archive/task/.../config` (экспорт GET + DELETE удаления + POST/DELETE экземпляра), `project/list`, `source/kafka/topics`, `feature-settings/value`, конфиг овердрафта (`archive/task/overdraft/config`), действия над экземпляром (`instance/resume`, `instance/suspend`, `instance/overdraft`, `instance/overdraft/reset`, `zone/{z}/instance/overdraft/reset/all`), метки (PUT `archive/index/.../labels`), ограничения (`archive/restrictions` GET/PATCH, `archive/restrictions/overview` GET, `archive/restrictions/(index|project)/{id}` GET/PATCH/DELETE), квоты экземпляра (`archive/quota/project/{project}` GET, `archive/task/project/{project}/quota/estimate` POST, `archive/task/overdraft/estimate` POST, `instance/quotas` PUT). После снятия «тихих» fallback'ов dev держится именно на этих роутах - добавляя новый боевой эндпоинт, сразу клади сюда mock-роут.
...
> ⚠️ Эндпоинтов **Ограничений** (`/archive/restrictions*`) в OpenAPI-снапшоте нет, но весь контракт (overview, значение/PATCH/DELETE по индексу и проекту, глобальное) подтверждён реальными запросами со стенда - см. «Форма управления Ограничениями». Эффекты на боевом паттерне (`failData → handleErrorFx`); dev-офлайн через API-моки (`ARMC_MOCK`). Реализация - `src/Entities/Restriction`.
...
- **Маппинг формы → запрос:** `FilterDrawer/mapFilters.ts` (`mapFormToArchiveFilters`). Подтверждены: `field:name` + все 4 select-оператора (IS=eq, IS NOT=isNot, IN=in, NOT IN=notIn); Скорость обработки = `field:maxOverdraftPercent` + операторы сравнения `ge`/`le`/`eq` (короткие ge/le, не gte/lte); Метки = `field:label` (ед.ч., не `labels`); Зона = `field:zone` (оператор `in`). Названия полей `name`/`status`/`label`/`zone`/`version`/`overdraft` сверены с исходником abyss `../mfe-abyss/src/containers/archive/components/FilterRow.tsx` (там перечислены все `field` фильтра архивов). **Грабля:** проект в abyss идёт как `field:shortName`, а у нас в коде - `field:project`; либо новый бэк переименовал поле, либо это баг - перепроверить при интеграции (живой запрос `archive/list/paginated` со стенда, параметр `filters`). Остальные числовые/диапазонные field (maxSizeBytes/maxStorageTimeSec/...) - предположение, помечено `TODO(archives-filter-api)`. Диапазоны from/to → gte/lte; единицы измерения (unit) пока **не конвертируются и не отправляются**.
...
- **Options:** Ключ проекта - `$optionsProject` (`Entities/Project`, грузится в `App.tsx`). Конфигурация/Проект/Метки/Зона - одним `GET /archive/list/filter-values` (`fetchArchivesFiltersFx` → `$archiveFilterValues: FilterItems`). Метки - оператор только `IN`/`NOT IN` (`IN_NOT_IN_OPERATOR_OPTIONS`). Статус/Зона - константы из `Shared/constants/filters.ts`. Грабли: `$optionsArchiveConfig` (дропдаун «По индексу» у ограничений) из `/filter-values` не накормить (нет id) - строится из постраничного `$archives`, см. `TODO(restrictions-index-options)`.
- **Фильтр в заголовке столбца** (`ArchivesTable/ColumnHeaderMultiSelectFilter.tsx`) - воронка `Icon.Filter` в шапке → `Popover` (поиск + чекбоксы значений + «Применить»/«Сбросить»), это «окно» из макета. Подключён через `createColumnHeaderFilter` в `columns.tsx`: Конфигурации - «Конфигурация» (`name`), «Ключ проекта» (`project`), «Метки» (`label`); Зоны - «Конфигурация» (`name`), «Зона» (`zone`), «Статус» (`status`). **Важно: это не локальный фильтр уровня 2 по загруженным строкам, а второй вход в тот же серверный фильтр уровня 1** - «Применить» пишет в `$appliedArchiveFilters` через `upsertFieldFilter(..., op='in')` (всегда оператор `in`), дальше как обычный фильтр уходит в `fetchArchivesFx` + сброс на 1-ю страницу + чип в тулбаре. Значения опций берёт из `$archiveFilterValues` (`getOptionsForField`/`filterFieldUtils.ts`), статус - статичный `STATUS_OPTIONS`. Активность отмечена точкой-маркером на воронке. Движковые column-filters data-grid при этом выключены (`enableColumnFilters={false}`).
- **Не сделано:** настоящий локальный фильтр уровня 2 (по уже загруженным строкам, отдельно от серверного) - сознательно не делаем, заголовочная воронка закрывает потребность через уровень 1; единицы измерения диапазонов; **сортировка по клику на заголовок** - см. ниже.
- **Сортировка по заголовку - не реализована.** На гриде `enableSorting={false}` (и в `defaultColumn`), `fetchArchivesFx` sort-параметров не принимает. abyss по `archive/list/paginated` сортировку **не слал** (только `pageSize/pageNumber/filters`, см. `../mfe-abyss` `ArchiveService.tsx`), поэтому формат sort-параметра у `paginated` неизвестен. Клиентская сортировка не годится (отсортировала бы только текущую страницу из N при серверной пагинации). Отложено до подтверждения контракта со стенда; делать серверной по образцу фильтров (sort в эффект + маппинг accessorKey→поле + `manualSorting` + состояние в стор/URL).
...
- **Меню действий экземпляра** (kebab в таблице Зоны) - `InstanceActionsCell.tsx`. Состав по макету: Старт/Стоп / Установить offset, Сбросить offset / Овердрафт / Переопределить квоты / Удалить. Подключён только **Старт/Стоп**: пункт один, зависит от `row.instanceStatus` - `RUNNING` → «Стоп» (`Icon.Pause`, action `suspend`), любой другой статус → «Старт» (`Icon.Play`, action `resume`). Пункт Старт/Стоп **скрыт при `instanceStatus === 'WITHOUT_RESPONSE'`** (статус неизвестен, `isStatusKnown`). При **открытии меню** (`DropdownMenu onStateChange`, open=true) дёргается `fetchInstanceStatusFx` для актуализации статуса строки. Клик по Старт/Стоп открывает `InstanceActionModal` (`src/pages/ArchivesPage/InstanceActionModal/`) - подтверждение через общий `ConfirmModal` (кнопки «Да»/«Отмена» через `cancelLabel="Отмена"`, текст «Вы уверены что, хотите запустить/остановить экземпляр архива {project}/{name}/{zone}?»). Стор открытия `$instanceActionModal` (`{ row, action } | null`) + `onOpenInstanceActionModal` / `onCloseInstanceActionModal`, reset на close и на `resumeInstanceFx.done`/`suspendInstanceFx.done`. Сетевой слой - `Entities/Instance/api.ts`: `resumeInstanceFx` (POST `.../instance/resume`) / `suspendInstanceFx` (POST `.../instance/suspend`), оба на стандартном паттерне API (`failData → handleErrorFx`). После `.done` через `sample`, а также при открытии меню дёргается `fetchInstanceStatusFx` (GET `.../instance/status`) - результат обновляет `status.indexing.status` нужного экземпляра в сторе `$archives` по `instance.id` (реакция-`.on` в `Entities/Archives/model.ts`). **Тост привязан к реальному статусу, а не к факту отправки команды**: эффект завершается на принятии команды (HTTP 200) - `.done` срабатывает, даже если экземпляр потом не дошёл до нужного статуса (`UNDEFINED`/`FAILED`), поэтому решение по тосту принимается по фактическому статусу. Сетевая ошибка самой команды (не 200) идёт в `handleErrorFx`, `.done` не срабатывает. Корреляция - опц. поле `notifyAction` в параметрах `fetchInstanceStatusFx` (ставится только в `sample` после resume/suspend, при рефетче по открытию кебаба - нет, чтобы не плодить тосты). `notifyInstanceActionResultFx` в `InstanceActionModal/model.ts` по `fetchInstanceStatusFx.done` (где `notifyAction` задан) решает: статус достиг ожидаемого (resume→`RUNNING`, suspend→`STOPPED`) - success «Экземпляр запущен/остановлен»; `FAILED` - error «Не удалось...»; иначе - warning «Команда отправлена, статус: {status}». Офлайн-fallback (`fallbackStatus`) возвращает ожидаемый статус, поэтому на моках тост остаётся success. У `fetchInstanceStatusFx` параметр `fallbackStatus` - что вернуть в офлайн-fallback: после действия ожидаемый (resume→RUNNING, suspend→STOPPED), при рефетче по открытию меню - текущий `row.instanceStatus` (чтобы строка не менялась без бэка). Остальные пункты меню - `TODO(instance-actions)`. Ролевую модель (видимость/дизейбл по правам) пока не трогаем - отложено до общей ролевой модели. **Второй вход в Старт/Стоп - сам бейдж статуса** (колонка «Статус» таблицы Зоны): `InstanceStatusCell.tsx` оборачивает `StatusBadge` и по клику дёргает тот же `onOpenInstanceActionModal` (`RUNNING` → suspend, иначе resume), `WITHOUT_RESPONSE` некликабелен. Поэтому `StatusBadge` теперь не чисто презентационный: `forwardRef` (нужен `Tooltip`) + флаг `clickable` (курсор/hover/focus, класс `statusBadgeClickable`); логику открытия модалки держит `InstanceStatusCell`, бейдж только рисует.
  - **Контракты остальных пунктов `TODO(instance-actions)`** (сняты со стенда, пути в axios-форме). Все экземплярные пути - база `/v1/internal/index/archive/task/project/{project}/name/{taskName}/zone/{zoneId}/instance`. После любого изменяющего действия - рефетч списка (`fetchArchivesFx` + при необходимости `fetchInstanceStatusFx`):
    - **Сбросить offset** - ✅ реализовано, `src/pages/ArchivesPage/ResetOffsetModal/`. `ConfirmModal` → `POST .../instance/reset` (без тела), стандартный паттерн API, рефетч `fetchArchivesFx` через `sample` в `Entities/Archives/model.ts`.
    - **Удалить** - ✅ реализовано, см. «Окно удаления экземпляра» ниже.
    - **Овердрафт** - ✅ реализовано, см. «Окно овердрафта экземпляра» ниже. Иконка-индикатор овердрафта в строке (зелёная/оранжевая/красная) пока не сделана - описана в разделе «Бизнес-логика».
    - **Переопределить квоты** - ✅ реализовано, см. «Дравер квот экземпляра» ниже.
    - **Установить offset** - ✅ реализовано, `src/pages/ArchivesPage/SetOffsetModal/`. Типы в `Entities/Instance/types.ts`. Данные: `GET /v1/flow/offset/project/{project}/task/{taskName}/zone/{zoneId}?businessTask=ARCHIVING` (путь `flow/offset`, не `index/archive`). Если данные пусты или у топика нет consumerGroups - тост «Отсутствуют источники данных для offset» + закрытие. UI: для каждого топика два поля read-only (имя/текущая CG) + два Select «Копировать из» / «Устанавливаемое значение». Режим «Зона» - ищет `consumerGroup.name` в `consumerGroups` по `indexName+projectName+type+zoneId`. POST body: `[{ sourceConsumerGroupId, topicName, projectName }]` только где задан `sourceConsumerGroupId`; `topicName`/`projectName` - split по точке из `item.topicName`. Перед POST - экран подтверждения внутри того же Modal (паттерн двух компонентов: `SetOffsetModal` outer + `SetOffsetContent` inner c `key={rowId-offsetData.length}`). mock-роуты: GET `flow/offset/...` → `offsetDataFixture` (реальный ответ стенда от 2026-06-29); POST `instance/setOffsets` и `instance/reset` → `{}`.
...
- **Окно меток** (пункт «Метки») - `src/pages/ArchivesPage/LabelsModal/` (`index.tsx` - Modal, `model.ts` - стор открытия). Состояние открытия по паттерну `$restrictionDeleteRow`: стор `$labelsModalRow` (строка-конфигурация или null) + события `onOpenLabelsModal(row)` / `onCloseLabelsModal`; закрывается на `onCloseLabelsModal` и на `saveLabelsFx.done`. Метки редактируются как чипы `Tag closable` + инпут (добавление по Enter, trim + дедуп), форма на react-hook-form (поля `labels: string[]`, `newLabel`), «Сохранить» активна пока набор отличается от исходного. Дедуп и сравнение наборов (`isSameLabels`) сейчас **регистрозависимые** - `Audit` и `audit` считаются разными метками; `TODO(labels)`: уточнить у аналитика, так и надо или нет. Метки инициализируются из `row.labels` (без отдельного GET). Монтируется в `ArchivesContentPage` рядом с `DrawerRestriction`/`FilterDrawer`. Сетевой слой - `Entities/Label/api.ts`: `saveLabelsFx` (PUT `.../labels`, заменяет весь набор), стандартный паттерн API с `handleErrorFx`; mock-роут PUT `.../labels` в `Shared/api/mock`.
- **Окно добавления экземпляра** (пункт «+ экземпляр») - `src/pages/ArchivesPage/AddInstanceModal/` (`index.tsx` - Modal, `model.ts` - стор открытия). Тот же паттерн, что у LabelsModal: стор `$addInstanceModalRow` + `onOpenAddInstanceModal(row)` / `onCloseAddInstanceModal`, закрытие на close и на `addInstanceFx.done`, тост «Экземпляр добавлен». Поля: read-only `TextField` с названием конфигурации (`row.configuration`) и `Select` зоны через `ControllerSelectSingle` (опции - константа `ZONE_OPTIONS` из `Shared/constants/filters.ts`, дефолт `PRIMARY`; форма в `FormProvider`). Сетевой слой - `Entities/Instance/api.ts`: `addInstanceFx({ project, taskName, zoneId })` (POST `.../zone/{zoneId}/instance` без тела), стандартный паттерн API с `handleErrorFx`. Монтируется в `ArchivesContentPage`.
...
- **Окно удаления конфигурации** (пункт «Удалить») - `src/pages/ArchivesPage/DeleteConfigModal/` (`index.tsx` - Modal-подтверждение, `model.ts` - стор открытия). Тот же паттерн: стор `$deleteConfigModalRow` + `onOpenDeleteConfigModal(row)` / `onCloseDeleteConfigModal`, закрытие на close и на `deleteArchiveFx.done`, тост «Архив удалён». Текст подтверждения: «Вы уверены что, хотите удалить архив {projectKey}/{configuration}? Его будет невозможно восстановить.» (имя архива = `project/taskName`, как в abyss). Кнопка удаления - `view="negative"`. Сетевой слой - `deleteArchiveFx` в `Entities/Archives/api.ts` (DELETE `.../project/{project}/name/{taskName}/config`), стандартный паттерн API с `handleErrorFx`. **После удаления список перезапрашивается:** в `Entities/Archives/model.ts` запоминаются параметры последней загрузки (`$lastFetchArchivesParams` / `$lastFetchCountFilters` через `.on(fetchArchivesFx, ...)`), и по `deleteArchiveFx.done` повторно дёргаются `fetchArchivesFx` (та же страница) + `fetchArchivesCountFx` - воспроизводит поведение abyss (после DELETE летят `archive/list/paginated` и `archive/list/page-count`). Монтируется в `ArchivesContentPage`.
- **Окно удаления экземпляра** (пункт «Удалить» в меню экземпляра) - `src/pages/ArchivesPage/DeleteInstanceModal/` (`index.tsx` - подтверждение, `model.ts` - стор открытия). Паттерн как у `DeleteConfigModal`: стор `$deleteInstanceModalRow` (`ArchiveInstanceView | null`) + `onOpenDeleteInstanceModal(row)` / `onCloseDeleteInstanceModal`, закрытие на close и на `deleteInstanceFx.done`, тост «Экземпляр удалён». Текст: «Вы уверены что, хотите удалить архив {projectName}/{configName}/{zoneId}? Его будет невозможно восстановить.», кнопка `view="negative"` (общий `ConfirmDeleteModal`). Сетевой слой - `Entities/Instance/api.ts`: `deleteInstanceFx({ project, taskName, zoneId })` (DELETE `.../zone/{zoneId}/instance` без тела), стандартный паттерн API с `handleErrorFx`. **После удаления** по `deleteInstanceFx.done` рефетчится только `fetchArchivesFx` (та же страница; число конфигураций не меняется, поэтому `fetchArchivesCountFx` не дёргаем) - `sample` в `Entities/Archives/model.ts`. Пункт меню привязан в `InstanceActionsCell.tsx`, модалка монтируется в `ArchivesContentPage`. (Массовое удаление по чекбоксам - отдельно, `deleteArchivesInstancesFx` в тулбаре.)
- **Окно овердрафта экземпляра** (пункт «Овердрафт» в меню экземпляра, одиночный экземпляр - аналог `ArchiveOverdraftDialog` из abyss) - `src/pages/ArchivesPage/InstanceOverdraftModal/` (`index.tsx` - Modal, `model.ts` - стор открытия). Стор `$instanceOverdraftModalRow` (`ArchiveInstanceView | null`) + `onOpenInstanceOverdraftModal(row)` / `onCloseInstanceOverdraftModal`, reset на close и на `changeInstanceOverdraftFx.done`/`resetInstanceOverdraftFx.done`. Поля: read-only «Экземпляр задачи» = `project / name / zone`; `ControllerInputNumber` «Процент увеличения скорости обработки» (префилл `row.overdraftPercent`, валидация `0 ≤ % ≤ maxOverdraft`, хелпер с максимумом); read-only «Максимальная скорость обработки» = `formatSpeed(row.maxDataRateBytesPerSec * (1 + 0.01 * percent))`, пересчёт на лету через `useWatch`. Потолок `maxOverdraft = min(overdraftConfig.maxOverdraftPercent, row.metadata.maxAvailableOverdraft)`; `maxDataRateBytesPerSec`/`maxAvailableOverdraft`/`overdraftPercent` уже есть в строке (доп. GET конфигурации не нужен, в отличие от abyss). Две кнопки футера, обе через общий `ConfirmModal` (локальный `useState<'change'|'reset'|null>`): «Изменить» (после `handleSubmit`) → `changeInstanceOverdraftFx`, «Сбросить овердрафт» → `resetInstanceOverdraftFx`. Сетевой слой - `Entities/Instance/api.ts`: `changeInstanceOverdraftFx` (POST `.../instance/overdraft` body `{ overdraftPercent }`) / `resetInstanceOverdraftFx` (POST `.../instance/overdraft/reset` без тела), оба на стандартном паттерне API (`failData → handleErrorFx`); тосты «Скорость обработки изменена»/«Овердрафт сброшен» в `model.ts`. После `.done` обоих рефетчится `fetchArchivesFx` (та же страница) - `sample` в `Entities/Archives/model.ts`. Потолок процента - новая мини-сущность `Entities/Overdraft` (`fetchOverdraftConfigFx` GET `.../task/overdraft/config`, стандартный паттерн API с `handleErrorFx`, возвращает `AxiosResponse<OverdraftConfig>`; стор `$overdraftConfig`, при недоступном конфиге остаётся `null` и потолок берётся из `maxAvailableOverdraft`), грузится при открытии диалога (`sample` на `onOpenInstanceOverdraftModal`). Пункт меню в `InstanceActionsCell.tsx` **задизейблен при `row.metadata.maxAvailableOverdraft === 0`** (как в abyss), модалка монтируется в `ArchivesContentPage`. (Массовый овердрафт по чекбоксам - отдельный `OverDraftModal`, см. ниже.)
- **Окно массового овердрафта** (нижний тулбар выбора по чекбоксам вкладки Зоны, кнопка `SpeedIcon` в `ArchivesActionsToolBar.tsx`) - `src/pages/ArchivesPage/OverDraftModal/`. Устанавливает **один процент сразу всем выбранным** экземплярам (вид отличается от одиночного окна: нет превью «макс. скорость», т.к. у экземпляров разный `maxDataRateBytesPerSec`; вместо этого - список выбранных). Стор `$overDraftModalRows` (`ArchiveInstanceView[] | null`) + `onOpenOverDraftModal(rows)` / `onCloseOverDraftModal`, reset на close и на `changeInstancesOverdraftFx.done`. Поле «Процент увеличения скорости обработки» (`ControllerInputNumber`, валидация `0 ≤ % ≤ maxOverdraft`); префилл - общий процент, если у всех выбранных одинаков, иначе 0. Потолок `maxOverdraft = min(overdraftConfig.maxOverdraftPercent, min(maxAvailableOverdraft по выбранным))` (тот же `$overdraftConfig`, грузится при открытии). Две кнопки в футере (как в одиночном окне, общий `ConfirmModal` через локальный `useState<'change'|'reset'|null>`): «Сохранить» («...изменить скорость обработки выбранных экземпляров (N)?») → `changeInstancesOverdraftFx`; «Сбросить овердрафт» («...сбросить ... выбранных экземпляров (N) до значения по умолчанию?») → `resetInstancesOverdraftFx`. Оба в `Entities/Instance/api.ts` - `Promise.allSettled` по выбранным (POST `.../instance/overdraft` body `{ overdraftPercent }` / POST `.../instance/overdraft/reset`); если хоть один запрос упал - ошибка пробрасывается в `failData → handleErrorFx`, `.done` не срабатывает (модалка и выделение остаются). После успешного `.done`: тост («Скорость обработки изменена» / «Овердрафт сброшен»), рефетч `fetchArchivesFx` (общий overdraft-`sample` в `Entities/Archives/model.ts`), снятие выделения чекбоксов (`setRowSelection({})` из `Features/TableView/model`). Этот массовый сброс бьёт **по выбранным** экземплярам (N запросов) - не путать со **сбросом по зоне** (см. ниже).
- **Окно сброса овердрафта по зоне** (`src/pages/ArchivesPage/ResetZoneOverdraftModal/`) - сброс у **всех** экземпляров выбранной зоны одним запросом (`reset/all`), независимо от выделения. Точка входа - **существующая** кнопка `Button.Icon` с иконкой перечёркнутой скорости `SpeedIconRemoved` + `Tooltip` «Сбросить овердрафт по зоне» в верхней панели у фильтров (`ArchivesTableToolBar.tsx`). Эта кнопка раньше ошибочно вешалась на «Сбросить фильтры» - иконка перечёркнутой скорости семантически = сброс овердрафта, а сброс фильтров и так есть на чипах («Сбросить все» в `ArchivesFilterChips`). Отдельную кнопку под овердрафт не заводим. Показывается на обеих вкладках (как и было). Стор открытия `$resetZoneOverdraftOpen` (bool) + `onOpenResetZoneOverdraftModal` / `onCloseResetZoneOverdraftModal`, reset на close и на `resetZoneOverdraftFx.done`. Окно 1 (макет): `Select` зоны (`ZONE_OPTIONS`, placeholder «Выберите зону») + «Выбрать» (disabled пока зона не выбрана) / «Отмена». «Выбрать» → `ConfirmModal` («Сбросить овердрафт? / Скорость обработки всех экземпляров будет сброшена до значений по умолчанию») → `resetZoneOverdraftFx({ zoneId })` (`Entities/Instance/api.ts`: POST `.../zone/{zoneId}/instance/overdraft/reset/all`, стандартный паттерн API с `handleErrorFx`). После `.done`: тост «Овердрафт сброшен», рефетч `fetchArchivesFx` (общий overdraft-`sample`). Mock-роут POST `.../zone/{zoneId}/instance/overdraft/reset/all` в `Shared/api/mock/index.ts`.
- **Массовый старт/стоп экземпляров** (нижний тулбар выбора по чекбоксам, `ArchivesActionsToolBar.tsx`, только вкладка Зоны) - **две** кнопки `Icon.Play` (Старт) / `Icon.Pause` (Стоп) с `Tooltip`. Действуют **по подмножеству** выбранных: Старт резьюмит тех, кто **не** `RUNNING` и не `WITHOUT_RESPONSE` (`startableInstances`); Стоп гасит только `RUNNING` (`stoppableInstances`). Кнопка **дизейблится**, если её подмножество пустое (Стоп неактивна без `RUNNING`, Старт - без подходящих). Клик → общий `ConfirmModal` («...запустить/остановить выбранные экземпляры (N)?», N = размер подмножества) → `resumeInstancesFx` / `suspendInstancesFx` (`Entities/Instance/api.ts`: `Promise.all` по подмножеству, на каждый экземпляр POST `.../instance/resume` / `.../instance/suspend` **+ сверка фактического статуса** GET `.../instance/status` с ожидаемым (`RUNNING`/`STOPPED`), как в одиночном меню; эффект возвращает `InstanceActionResult[]` = `{ ...params, reachedTarget }`. **Ошибки тут НЕ через `handleErrorFx`** (в отличие от остальных мутаций): провалы агрегируются и показываются одним тостом в тулбаре, поэтому POST остаётся с внутренним `.catch`, а статус сверяется отдельно. При недоступном статусе (офлайн) `reachedTarget: true`, чтобы на моках не было ложных провалов. После выполнения (инлайн в тулбаре): снятие выделения + **один агрегированный тост** - все ок → success «Экземпляры запущены/остановлены»; есть провалы → warning «Запущено/Остановлено K из N» + список не достигших ожидаемого статуса в `description`. По тосту на экземпляр сознательно НЕ плодим. Плюс рефетч `fetchArchivesFx` (отдельный `sample` в `Entities/Archives/model.ts`, обновляет грид). Async-нюанс: статус сразу после команды бывает промежуточным → возможен ложный «не удалось» (тот же риск, что и в одиночном окне). Mock-роуты POST `.../instance/resume` / `.../instance/suspend` в `Shared/api/mock/index.ts`; для GET `.../instance/status` mock-роута нет (статичный статус ломал бы сверку), офлайн работает через fallback.
...
### Дравер квот экземпляра (`src/pages/ArchivesPage/InstanceQuotasModal/`)

- **Точка входа:** пункт «Переопределить квоты» в меню экземпляра (`InstanceActionsCell.tsx`). Доступен только при включённом feature-флаге лимитов (`$isLimitFeatureSettingEnabled`). Монтируется в `ArchivesContentPage.tsx` как `<DrawerInstanceQuotas />`.
- **Паттерн двух компонентов:** `DrawerInstanceQuotas` (внешний) читает `$instanceQuotasModalRow` и рендерит `<Drawer width="80%">`; `DrawerInstanceQuotasContent` (внутренний) монтируется с `key={row.id}` - при смене экземпляра пересоздаётся, инициализируя форму и единицы из пропсов без `useEffect` (иначе `react-hooks/set-state-in-effect`).
- **Стейт:** `$instanceQuotasModalRow` + `onOpenInstanceQuotasModal(row)` / `onCloseInstanceQuotasModal`. При открытии автоматически стартуют `fetchArchiveConfigFx` (чтобы получить kafka-топики, которые нужны estimate) и `fetchCurrentProjectLimitsFx` (правая колонка дравера). Топики оседают в `$instanceQuotasTopics`.
- **Форма:** три поля - `maxDataRateBytesPerSec`, `maxSizeBytes`, `maxStorageTimeSec`, каждое с парным `Select` единицы (скорость: B/s / KB/s / MB/s; размер: MB / GB / TB; время: SEC / MIN / HOURS / DAYS / WEEKS / MONTH). Дефолты из строки экземпляра. Форма на `useForm` внутри `DrawerInstanceQuotasContent`, не через `FormProvider` (локальная форма).
- **Эстимейт:** `useEffect` по `[watchedValues, topics, units, row.projectName, row.configName]` дёргает `fetchInstanceEstimateFx` и `fetchInstanceOverdraftEstimateFx`. Результат рендерится в правой колонке: `LimitsProjectInfo` (лимиты проекта) и `LimitsInfo` (таблица расчёта).
- **Сохранение:** `handleSubmit` → `saveInstanceQuotasFx` в `Entities/Instance/api.ts` (PUT `.../instance/quotas?maxSizeBytes=&maxDataRateBytesPerSec=&maxStorageTimeSec=` - **параметры в query, тела нет**). Кнопка «Сохранить» дизейблится при `hasBlockers` (есть блокеры из эстимейта и флаг лимитов включён). После `.done` дравер закрывается + тост «Квоты сохранены» + рефетч `fetchArchivesFx`.
- **Эффекты эстимейта для дравера:** `fetchInstanceEstimateFx` - отдельный (в `Entities/Limits/api.ts`), чтобы не ломать `StepLimits` c его `fetchCurrentEstimateFx`. А вот overdraft-эстимейт **общий**: `fetchInstanceOverdraftEstimateFx` - это алиас `fetchCurrentOverdraftEstimateFx` (тот же эффект, не отдельный). Estimate-сторы: `$currentProjectEstimate` (слушает оба estimate-эффекта c чуть разной логикой `estimateBySize`; в него же `fetchCurrentOverdraftEstimateFx.doneData` кладёт `maxOverdraftPercent`), `$currentEstimateWarnings`, `$currentEstimateBlockers`. Отдельного стора `$currentEstimateOverdraft` больше нет - потолок овердрафта живёт полем `maxOverdraftPercent` в `$currentProjectEstimate`.
- **`fetchArchiveConfigFx`** в `Entities/Archives/api.ts` (GET `.../name/{taskName}/config` с типизацией `ArchiveConfigPayload`) - нужен для получения kafka-топиков (`source.kafka[]`) и для загрузки конфигурации в форму редактирования. `exportArchiveConfigFx` отдельно (тот же эндпоинт, но ответ не типизируем - скачиваем как есть).

### Лимиты и квоты (`Entities/Limits`, `Features/Limits/ui`)

- **`Entities/Limits`:** эффекты `fetchCurrentProjectLimitsFx` (`GET /v1/index/archive/quota/project/{project}`), `fetchCurrentEstimateFx` (`POST /v2/.../quota/estimate`, принимает `QuotaEstimateRequestParams` с реальными параметрами - не MOCK), `fetchCurrentOverdraftEstimateFx` (`POST /v1/internal/index/archive/task/overdraft/estimate`, ответ `OverdraftEstimateItem = { maxOverdraftPercent }`). Для дравера квот экземпляра: `fetchInstanceEstimateFx` (отдельный, тип params `InstanceEstimateRequestParams`) и `fetchInstanceOverdraftEstimateFx` (**алиас** `fetchCurrentOverdraftEstimateFx`, не отдельный эффект). Все на стандартном паттерне API с `handleErrorFx`.
- **`Features/Limits/ui`:** `LimitsInfo`, `LimitsProjectInfo` - таблицы расчёта квот/лимитов (используются в `StepLimits` и в драйвере квот экземпляра `DrawerInstanceQuotasContent`).
